...
const WriteArticle = () => {
  ...
  const onSubmitHandler = async (e) => {
     ...
     try {
      setLoading(true);
      const prompt = `Write an article about ${input} with a word count of around ${selectedLength.text} words.`;
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-xl border border-gray-200'>
        ...
        <input onChange={(e) => setInput(e.target.value)} value={input} type="text" className='border border-gray-300 rounded-md w-full p-2 px-3 outline-none text-sm '
        placeholder='The future of Artificial Intelligence' required/>

        <p className='mt-4 text-sm font-medium'>Article Length</p>
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {articleLength.map((item, index) => (
            <span onClick={() => setSelectedLength(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength.text === item.text ? 'bg-blue-50 border-blue-700 text-blue-700' : 'text-gray-500 border-gray-300'}`} 
        ...
        border-gray-200 min-h-96 max-h-[600px]'>
          ...
          {!content ? (<div className='flex-1 flex justify-center items-center'>
            <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
              ...
              <p>Enter a topic anc click "Generate Article" to get started</p>
            </div>

          </div>
          ) : (
            <div className='mt-3 h-full overflow-y-scroll pr-2 text-sm text-gray-600'>
