  ...
  const onSubmitHandler = async (e) => {
      ...
      try {
        ...
        if(object.split(' ').length > 1) {
          toast.error("Please provide only a single object name.");
          setLoading(false);
          return; 
        }
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-xl border border-gray-200'>
        ...
        <textarea onChange={(e) => setObject(e.target.value)} value={object} rows={4} className='border border-gray-300 rounded-md w-full p-2 px-3 outline-none text-sm '
