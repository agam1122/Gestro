  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-xl border border-gray-200'>
        ...
        <input onChange={(e) => setInput(e.target.files[0])} value={input} type="file" accept='image/*'
         className='border border-gray-300 rounded-md w-full p-2 px-3 outline-none text-sm text-gray-600' required/>

        <p className='mt-6 text-sm font-medium'>Describe object name to remove</p>
        <textarea onChange={(e) => setInput(e.target.value)} value={object} rows={4} className='border border-gray-300 rounded-md w-full p-2 px-3 outline-none text-sm '
